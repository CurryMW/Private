...
本项目每天收集最新的 AI 技术动态，过滤重复内容和纯商业新闻，再调用兼容 OpenAI Chat Completions 接口的模型，从候选内容中筛选并生成最多 8 条中文摘要，最后推送到钉钉自定义机器人。项目可部署在 GitHub 私有仓库中，通过 GitHub Actions 每天北京时间 08:30 开始自动运行，并在 08:40、08:50 提供补偿触发机会。
...
`Daily DingTalk Digest` 工作流使用三个 cron 表达式：`30 0 * * *`、`40 0 * * *`、`50 0 * * *`，分别表示每天 UTC 00:30、00:40、00:50，也就是 `Asia/Shanghai` 时区的 08:30、08:40、08:50。定时工作流从默认分支运行，因此 `.github/workflows/daily.yml` 必须存在于默认分支（当前仓库为 `master`），并且仓库 Actions 必须保持启用。
...
如果某次定时运行得到 `status=empty` 或运行失败，不会记录当日已完成，08:40 或 08:50 的后续触发仍会重新尝试。GitHub 没有创建某个 cron 运行时，其他两个 cron 是独立的补偿机会。
...
GitHub cron 不是精确调度器，平台负载较高时可能延迟。确认 Actions 已启用，`daily.yml` 位于仓库默认分支，并且三个 cron 仍是 `30 0 * * *`、`40 0 * * *`、`50 0 * * *`。不要把 cron 改成本地时间，因为 GitHub cron 使用 UTC。如果三个定时运行都没有出现，需要及时推送时，可以手动预演后再启动正式任务。
