...
本项目包含两个相互独立的流程：`AI 情报摘要`只通过百度官方搜索 API 发现候选，再由 TeamoRouter `gpt-5.6-luna` 生成最多 8 条中文摘要；`GitHub 趋势报告`使用 GitHub 官方 API 维护独立基线，观察周期满 72 小时后生成最多 5 个项目的报告。AI 情报工作流每天北京时间 08:30 运行；信息范围限于百度可检索的公开内容，不代表全球或社交平台完整覆盖。
...
`AI 情报摘要` 工作流只使用一个 cron 表达式 `30 0 * * *`，表示每天 UTC 00:30，也就是 `Asia/Shanghai` 时区的 08:30。定时工作流只会在相关改动推送到默认分支且仓库 Actions 启用后生效。
...
每次 AI 情报运行最多发起 20 次百度搜索（16 个固定中英文主题加 4 个轮换热点），只处理最近 7 天的候选。同一事件 3 天内不重复推送；3 天后出现实质性新进展时可以再次入选。没有合格内容时任务成功结束但钉钉保持静默，不写成功状态；搜索、模型或钉钉失败时任务非零退出，不发送错误、降级或运行状态通知。百度限额或免费额度耗尽也按搜索失败处理，不自动改用后付费或其他搜索来源。独立用量账本还会跨运行执行每日 20 次累计上限。
...
- `first_party_domains` 用于识别事件主体；没有第一方证据时，只要标题和原文片段直接支持同一事实即可作为“未经第一方确认”内容入选；多个可信来源相互冲突时仍会被拒绝。
...
| `WINDOW_HOURS` | 否 | `168` | 正整数，百度搜索候选的允许时间窗口；默认最近 7 天。 |
| `EVENT_DEDUPE_DAYS` | 否 | `3` | 正整数，同一事件的去重天数；超过后仅有实质性新进展才重新入选。 |
| `MAX_ITEMS` | 否 | `8` | 1 到 8 之间的整数；模型返回超过该数量会校验失败。 |
| `TIMEZONE` | 否 | `Asia/Shanghai` | 报告日期使用的 IANA 时区；未知时区会导致运行失败。 |
| `DRY_RUN` | 否 | `false` | 不区分大小写的 `1`、`true`、`yes` 或 `on` 表示真，其他值表示假。预演模式只打印内容，不发送或保存已发送日报状态；百度搜索用量仍会计入独立账本。 |
...
返回的线索会先按 7 天窗口、30 天 URL 历史、3 天事件历史、每站最多 3 条和证据等级处理，最多向模型提交 40 条。搜索数据始终视为不可信输入，模型返回的来源和 URL 由程序重新绑定到本次候选。
...
这表示最近 7 天没有获取到通过证据规则的合格候选。这是成功但静默的运行：程序不调用钉钉，不回放旧内容，也不记录当日成功状态。
