...
def _heading(report_date: date, part: int, total: int) -> str:
    suffix = "" if total == 1 else f"（{part}/{total}）"
    return f"# AI 技术日报｜{report_date.isoformat()}{suffix}"
...
def _compose_part(
    ...
    sections = [_heading(report_date, part, total)]
    ...
    if trends is not None:
        ...
        sections.append(f"信息范围：最近 {window_hours} 小时")
    return "\n\n".join(sections)
